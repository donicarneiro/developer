...
$query = $pdo->prepare("INSERT INTO vendas set valor = '$total_venda', usuario = '$id_usuario', pagamento = :pagamento, lancamento = :lancamento, data_lanc = CurDate(), data_pgto = :data, desconto = :desconto, acrescimo = :acrescimo, subtotal = :subtotal, parcelas = :parcelas, status = '$status', cliente = :cliente, valor_custo = '$total_custo'");
...
if($status == 'Concluída'){
	...
	if($parcelas > 1){
		...
		$query = $pdo->query("INSERT INTO contas_receber set descricao = '$descricao_conta', cliente = '$cliente', entrada = '$lancamento', documento = '$pagamento', plano_conta = 'Venda', data_emissao = curDate(), vencimento = '$data', frequencia = 'Uma Vez', valor = '$subtotal', usuario_lanc = '$id_usuario', status = 'Pendente', data_recor = curDate(), id_venda = '$id_ult_registro'");
	}
}
